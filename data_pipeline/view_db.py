...
def main():
    with Session(db.engine) as session:
        ...
        # Artículos por jugador (join with players table)
        print("=" * 60)
        print("👤 ARTÍCULOS POR JUGADOR (Top 10)")
        print("=" * 60)

        results = session.execute(
            select(db.PlayerRow.name, db.PlayerRow.club, func.count().label("total"))
            .join(db.ArticleRow, db.ArticleRow.player_id == db.PlayerRow.id)
            .group_by(db.PlayerRow.id)
            .order_by(func.count().desc())
            .limit(10)
        ).all()

        for name, club, count in results:
            print(f"{name:20} ({club:15}) {count:3} artículos")
